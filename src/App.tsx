...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF Sob Medida (v1.9.5)...');
    setSyncStatus('syncing');

    const originalViewport = getViewport();

    try {
      // 1. RESET DE CÂMERA: Fundamental para que as coordenadas dos nós sejam lidas de forma absoluta
      setViewport({ x: 0, y: 0, zoom: 1 }, { duration: 0 });
      await new Promise(resolve => setTimeout(resolve, 100));

      // 2. CÁLCULO MANUAL DE LIMITES (Bounding Box Robusta)
      // Percorremos todos os nós para encontrar o retângulo que contém todos eles
      let minX = Infinity;
      let minY = Infinity;
      let maxX = -Infinity;
      let maxY = -Infinity;
      ...
      nodes.forEach((node) => {
        ...
        // Consideramos larguras/alturas padrão caso não estejam definidas no objeto
      ...
      // Adicionamos uma pequena margem de segurança interna (buffer)
      const buffer = 20;
      const bounds = {
        x: minX - buffer,
        y: minY - buffer,
      ...
      // 3. DEFINIÇÃO DE ESPAÇAMENTOS (Padding e Título)
      const padding = 60;      
      const titleHeight = 100; 
      
      // 4. DIMENSÕES DO CANVAS VIRTUAL
      ...
      // 6. CAPTURA COM ALINHAMENTO FORÇADO
      // O segredo aqui é o 'transform': ele "puxa" o desenho para o canto superior esquerdo do canvas
      // compensando a posição original (bounds.x) e adicionando o padding/título
      const dataUrl = await toPng(flowElement, {
        ...
        style: {
          ...
          transform: `translate(${-bounds.x + padding}px, ${-bounds.y + padding + titleHeight}px) scale(1)`,
        },
      });

      // 7. RESTAURAR INTERFACE
