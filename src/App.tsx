...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.2 - Lógica Estável Revertida)...');
    setSyncStatus('syncing');
    ...
    try {
      ...
      // 2. CAPTURA DIRETA (Lógica que "funcionava" anteriormente)
      // Usamos toJpeg diretamente no elemento visível para máxima compatibilidade
      ...
      // 3. GERAR PDF
      // Ajustamos o tamanho do PDF para o tamanho do diagrama capturado
      const pdf = new jsPDF({
        orientation: bounds.width > bounds.height ? 'landscape' : 'portrait',
        unit: 'px',
        format: [bounds.width + padding * 2, bounds.height + padding * 2 + 80]
      });

      // Adicionar a imagem
      pdf.addImage(dataUrl, 'JPEG', padding, padding + 60, bounds.width, bounds.height);
      ...
      pdf.text(title, (bounds.width + padding * 2 - titleWidth) / 2, 40);

      const timestamp = gerarTimestamp();
      pdf.save(`fluxograma-${projetoAtivo?.nome || 'projeto'}-${timestamp}.pdf`);
      
      addDebugLog('PDF v2.0.2 gerado com sucesso!');
      setSyncStatus('synced');
    } catch (error) {
      console.error('Erro ao exportar PDF:', error);
      addDebugLog('Erro na exportação: ' + (error instanceof Error ? error.message : 'Erro desconhecido'));
      setSyncStatus('error');
      alert('Houve um problema ao gerar o PDF. Tente ajustar o zoom do diagrama e tente novamente.');
    }
