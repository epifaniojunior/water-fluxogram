...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.39 - Otimização Máxima)...');
    setSyncStatus('syncing');

    // Injetar estilos de força bruta para garantir visibilidade
    ...
    styleEl.innerHTML = `
      .react-flow__node, .react-flow__edge, .react-flow__edge-path, .react-flow__viewport {
        opacity: 1 !important;
    ...
    try {
      // 1. CALCULAR ÁREA REAL DOS NÓS
      const nodesRect = getRectOfNodes(nodes);
      const padding = 60; // Padding reduzido para aproveitar mais espaço
      
      // Dimensões da captura
      const captureWidth = nodesRect.width + padding * 2;
      const captureHeight = nodesRect.height + padding * 2;

      // Aguardar renderização dos estilos injetados
      await new Promise(resolve => setTimeout(resolve, 1000));

      // 2. CAPTURAR USANDO PNG COM DIMENSÕES CALCULADAS
      ...
      // 3. LIMPAR ESTILOS TEMPORÁRIOS
      if (styleEl) styleEl.innerHTML = '';

      // 4. GERAR PDF A4 LANDSCAPE
      const pdf = new jsPDF({
      ...
      const availableH = pageHeight - margin - (titleY + titleHeight);
      
      const img = new Image();
      img.src = dataUrl;
      await new Promise(resolve => (img.onload = resolve));
      
      // Cálculo de escala para ocupar o MAIOR espaço possível
      const aspect = img.width / img.height;
      let imgW = availableW;
      let imgH = imgW / aspect;
      
      // Se a altura ultrapassar, ajustamos pela altura
      if (imgH > availableH) {
        imgH = availableH;
        imgW = imgH * aspect;
      }
      
      // Centralização final na área disponível
      const x = margin + (availableW - imgW) / 2;
      const y = (titleY + titleHeight) + (availableH - imgH) / 2;
      ...
      addDebugLog('PDF v2.0.39 gerado com sucesso (Otimização Máxima)!');
      setSyncStatus('synced');
    } catch (error) {
