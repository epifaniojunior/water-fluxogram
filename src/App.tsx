...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.1 - Solução de Compatibilidade Máxima)...');
    setSyncStatus('syncing');
    ...
    try {
      ...
      // Limite de segurança para o Canvas (evitar crash em dispositivos móveis)
      // Mobile browsers costumam limitar canvas a 4096px ou 8192px. 
      // Vamos usar 4000px como limite seguro de renderização.
      const maxCanvasDim = 4000;
      let renderScale = 1;
      if (virtualWidth > maxCanvasDim || virtualHeight > maxCanvasDim) {
        renderScale = maxCanvasDim / Math.max(virtualWidth, virtualHeight);
        addDebugLog(`Aviso: Diagrama grande. Ajustando escala de renderização para ${renderScale.toFixed(2)}x`);
      }

      // 2. PREPARAÇÃO DO ELEMENTO (v2.0.1)
      const flowElement = document.querySelector('.react-flow') as HTMLElement;
      if (!flowElement) throw new Error('Elemento React Flow não encontrado');

      // Criamos um container temporário que seja "visível" para o navegador mas não para o usuário
      const container = document.createElement('div');
      container.id = 'react-flow-export-v201';
      Object.assign(container.style, {
        ...
        zIndex: '-9999',
        opacity: '0.01', // Quase invisível mas "renderizável"
      ...
      // 3. AGUARDAR PROCESSAMENTO DO NAVEGADOR
      addDebugLog('Aguardando processamento de estilos e fontes...');
      await document.fonts.ready;
      // Delay maior para garantir que o clone seja "pintado" pelo navegador
      await new Promise(resolve => setTimeout(resolve, 800));

      // 4. CAPTURA USANDO CANVAS (Mais estável em Mobile)
      // toCanvas é frequentemente mais robusto que toPng/toJpeg direto
      const canvas = await toCanvas(container, {
        width: virtualWidth,
        height: virtualHeight,
        pixelRatio: renderScale * 2, // Multiplicamos por 2 para manter nitidez
        backgroundColor: '#f8fafc',
        cacheBust: true,
      });

      // Limpamos o DOM imediatamente
      document.body.removeChild(container);

      // Converter canvas para DataURL (JPEG é mais leve para PDFs grandes)
      const dataUrl = canvas.toDataURL('image/jpeg', 0.95);

      if (dataUrl.length < 10000) {
        throw new Error('A imagem gerada parece estar vazia. Tente novamente.');
      }
      ...
      setSyncStatus('error');
      alert('Não foi possível gerar o PDF. Verifique se o diagrama não é excessivamente grande ou tente em outro navegador.');
    }
