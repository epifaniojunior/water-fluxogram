...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF inteligente...');
    setSyncStatus('syncing');
    ...
    try {
      ...
      const padding = 100; // Margem de respiro
      
      // 2. Preparar a UI (ocultar elementos)
      ...
      // 4. Forçar estilos de segurança nos labels
      ...
      // 5. Ajustar a visão para o enquadramento perfeito antes da captura
      fitView({ duration: 0, padding: 0.1, includeHiddenNodes: true });
      await new Promise(resolve => setTimeout(resolve, 800));

      // 6. Captura usando toPng
      // No mobile, forçamos um tamanho de captura que caiba o desenho
      ...
      // 7. Restaurar a interface original
      ...
      // 8. Gerar o PDF com as dimensões da captura
      const pdf = new jsPDF({
  ...
  }, [nodes, projetoAtivo, addDebugLog, fitView, getViewport, setViewport]);
