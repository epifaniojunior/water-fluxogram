...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.4 - A4 Landscape Padronizado)...');
    setSyncStatus('syncing');

    try {
      // 1. CÁLCULO DE LIMITES DO DIAGRAMA
      ...
      // 3. GERAR PDF A4 LANDSCAPE
      // Dimensões A4 Landscape em mm: 297 x 210
      const pdf = new jsPDF({
      ...
      // Margens e Espaço para Título
      const margin = 10;
      const titleAreaHeight = 20;
      const availableWidth = pageWidth - (margin * 2);
      const availableHeight = pageHeight - (margin * 2) - titleAreaHeight;

      // Calcular dimensões da imagem mantendo o aspect ratio
      ...
      const yOffset = margin + titleAreaHeight + (availableHeight - imgHeight) / 2;
      ...
      addDebugLog('PDF v2.0.4 gerado com sucesso (A4 Landscape)!');
      setSyncStatus('synced');
    } catch (error) {
