...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.38 - Captura Universal)...');
    setSyncStatus('syncing');

    const originalViewport = getViewport();
    
    // Injetar estilos de força bruta
    ...
    styleEl.innerHTML = `
      ...
      /* Forçar visibilidade das arestas */
    ...
    try {
      // 1. CALCULAR ÁREA TOTAL DOS NÓS (Independente de Viewport)
      ...
      // 2. CAPTURAR USANDO PNG COM DIMENSÕES FIXAS (Captura Universal)
      // Isso ignora o tamanho da tela do dispositivo e foca apenas nos nós
      const dataUrl = await toPng(flowElement, {
        backgroundColor: '#ffffff',
        pixelRatio: 2, // 2 é suficiente para A4 e evita arquivos gigantes
        width: captureWidth,
      ...
      addDebugLog('PDF v2.0.38 gerado com sucesso (Captura Universal)!');
      setSyncStatus('synced');
    } catch (error) {
  ...
  }, [nodes, projetoAtivo, addDebugLog, getViewport]);
