...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF Sob Medida (v1.9.3)...');
    setSyncStatus('syncing');

    const currentViewport = getViewport();

    try {
      // 1. Medir as dimensões reais do desenho (Bounding Box)
      const nodesBounds = getRectOfNodes(nodes);
      
      // 2. Definir espaçamentos fixos
      const padding = 60;      // Espaço nas laterais e fundo
      const titleHeight = 80;  // Espaço reservado para o título no topo
      
      // 3. Calcular dimensões dinâmicas do Canvas Virtual
      // O canvas terá exatamente o tamanho do desenho + margens
      const virtualWidth = nodesBounds.width + (padding * 2);
      const virtualHeight = nodesBounds.height + (padding * 2) + titleHeight;

      // 4. Ocultar elementos da interface
      ...
      // 5. Captura de Alta Definição com Bounding Box
      const dataUrl = await toPng(flowElement, {
        ...
        style: {
          ...
          // Alinhamento preciso: movemos o desenho para o início do canvas + padding
          transform: `translate(${(-nodesBounds.x + padding)}px, ${(-nodesBounds.y + padding + titleHeight)}px) scale(1)`,
        },
      });

      // 6. Restaurar interface
      ...
      // 7. Gerar PDF com o tamanho EXATO do desenho
      const pdf = new jsPDF({
      ...
      // 8. Adicionar Título Centralizado
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(28);
      pdf.setTextColor(15, 23, 42);
      const title = (projetoAtivo?.nome || 'PROJETO').toUpperCase();
      const titleWidth = pdf.getTextWidth(title);
      // Centralizado horizontalmente em relação à largura real do desenho
      pdf.text(title, (virtualWidth - titleWidth) / 2, 50);
