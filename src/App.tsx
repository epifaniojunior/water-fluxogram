...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF Profissional (v1.9.7)...');
    setSyncStatus('syncing');

    try {
      // 1. CÁLCULO DE LIMITES REAIS (Bounding Box)
      // Usamos o tamanho real dos nós (210px de largura)
      let minX = Infinity;
      let minY = Infinity;
      let maxX = -Infinity;
      let maxY = -Infinity;
      ...
      nodes.forEach((node) => {
        ...
        const w = 210; // Largura real do nó no CSS
        const h = 180; // Altura estimada
      ...
      const buffer = 40; // Aumentado para garantir margem
      const bounds = {
        x: minX - buffer,
        y: minY - buffer,
      ...
      // 2. DIMENSÕES DO CANVAS VIRTUAL
      const padding = 80;      
      const titleHeight = 120; 
      const virtualWidth = bounds.width + (padding * 2);
      const virtualHeight = bounds.height + (padding * 2) + titleHeight;

      // 3. CAPTURA COM CLONE MANUAL
      // Criamos um clone do elemento para manipular sem afetar a UI do usuário
      const clone = flowElement.cloneNode(true) as HTMLElement;
      document.body.appendChild(clone);
      
      // Estilizamos o clone para o tamanho virtual
      ...
      // Localizamos a viewport interna no clone e forçamos o posicionamento
      ...
      // Removemos elementos indesejados do clone
