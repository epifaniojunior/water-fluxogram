...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF Profissional (v1.9.4)...');
    setSyncStatus('syncing');

    // 1. Salvar o estado original para restaurar depois
    const originalViewport = getViewport();

    try {
      // 2. RESET CRÍTICO: Zeramos o viewport interno para garantir que o snapshot 
      // capture as coordenadas reais dos nós sem interferência do zoom/pan atual do usuário
      setViewport({ x: 0, y: 0, zoom: 1 }, { duration: 0 });
      
      // Pequena pausa para o React Flow processar o reset de câmera
      await new Promise(resolve => setTimeout(resolve, 50));

      // 3. Medir as dimensões reais do desenho (Bounding Box)
      const nodesBounds = getRectOfNodes(nodes);
      
      // 4. Definir espaçamentos fixos
      const padding = 60;      // Margem lateral e inferior
      const titleHeight = 90;  // Espaço para o título no topo
      
      // 5. Calcular dimensões do Canvas Virtual
      const virtualWidth = nodesBounds.width + (padding * 2);
      const virtualHeight = nodesBounds.height + (padding * 2) + titleHeight;

      // 6. Ocultar elementos da interface
      ...
      if (searchBox) searchBox.parentElement!.style.display = 'none';
      panels.forEach(p => (p as HTMLElement).style.display = 'none');

      // 7. Captura de Alta Definição
      // Aplicamos o transform no clone do elemento para centralizar o desenho no canvas virtual
      const dataUrl = await toPng(flowElement, {
        ...
        style: {
          ...
          // Movemos o desenho (que agora está em coord 0,0) para a posição correta no canvas
          transform: `translate(${(-nodesBounds.x + padding)}px, ${(-nodesBounds.y + padding + titleHeight)}px) scale(1)`,
        },
      });

      // 8. Restaurar interface
      ...
      pdf.addImage(dataUrl, 'PNG', 0, 0, virtualWidth, virtualHeight);

      // 10. Adicionar Título Centralizado
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(28);
      pdf.setTextColor(15, 23, 42);
      const title = (projetoAtivo?.nome || 'PROJETO').toUpperCase();
      const titleWidth = pdf.getTextWidth(title);
      pdf.text(title, (virtualWidth - titleWidth) / 2, 55);
      ...
      // 11. Restaurar a visão original do usuário
      setViewport(originalViewport);
    }
  }, [nodes, projetoAtivo, addDebugLog, getViewport, setViewport]);
