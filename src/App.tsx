...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.5 - Padronização Multi-dispositivo)...');
    setSyncStatus('syncing');

    try {
      // 1. CÁLCULO DE LIMITES DO DIAGRAMA REAL
      const nodesBounds = getRectOfNodes(nodes);
      
      // 2. CONFIGURAÇÃO DO "ESTÚDIO DE CAPTURA" VIRTUAL
      // Usamos um tamanho fixo grande para garantir alta resolução e consistência independente do aparelho
      const captureWidth = 2400;
      const captureHeight = 1350; // Proporção 16:9 de alta definição
      
      const vContainer = document.createElement('div');
      vContainer.id = 'pdf-capture-studio-v205';
      Object.assign(vContainer.style, {
        position: 'fixed',
        top: '0',
      ...
      // 3. CLONAGEM E AJUSTE DE VIEWPORT NO CLONE
      const clone = flowElement.cloneNode(true) as HTMLElement;
      vContainer.appendChild(clone);
      ...
      if (viewport) {
        // Calcular escala ideal para caber no estúdio com margem de segurança
        const padding = 150;
        const availableW = captureWidth - (padding * 2);
        const availableH = captureHeight - (padding * 2);
        
        const scaleW = availableW / nodesBounds.width;
        const scaleH = availableH / nodesBounds.height;
        // Limitamos o zoom máximo em 1.5x para não pixelar, mas garantimos que caiba
        const finalScale = Math.min(scaleW, scaleH, 1.5);
        
        // Cálculo de centralização matemática
        const tx = (captureWidth - nodesBounds.width * finalScale) / 2 - nodesBounds.x * finalScale;
        const ty = (captureHeight - nodesBounds.height * finalScale) / 2 - nodesBounds.y * finalScale;
        
        viewport.style.transform = `translate(${tx}px, ${ty}px) scale(${finalScale})`;
      }

      // Ocultar elementos de interface que não devem sair no PDF
      const toHide = clone.querySelectorAll('.react-flow__controls, .react-flow__attribution, .search-input-container, .react-flow__panel');
      toHide.forEach(el => (el as HTMLElement).style.display = 'none');

      // 4. AGUARDAR PROCESSAMENTO DO BROWSER E CAPTURAR
      await document.fonts.ready;
      // Delay para garantir que o clone foi "pintado" internamente
      await new Promise(resolve => setTimeout(resolve, 1000));

      const dataUrl = await toJpeg(vContainer, {
        width: captureWidth,
        height: captureHeight,
        quality: 0.95,
        pixelRatio: 1 // 2400px já é resolução suficiente
      });

      // Limpeza imediata do DOM
      document.body.removeChild(vContainer);

      // 5. GERAR PDF A4 LANDSCAPE (297mm x 210mm)
      const pdf = new jsPDF({
      ...
      // Adicionar Título Centralizado no Topo
      ...
      // Calcular dimensões da imagem no PDF (Abaixo do título)
      const imgMargin = 10;
      const titleSpace = 20;
      const pdfAvailableW = pageWidth - (imgMargin * 2);
      const pdfAvailableH = pageHeight - (imgMargin * 2) - titleSpace;
      
      const captureAspect = captureWidth / captureHeight;
      let finalImgW = pdfAvailableW;
      let finalImgH = finalImgW / captureAspect;
      
      if (finalImgH > pdfAvailableH) {
        finalImgH = pdfAvailableH;
        finalImgW = finalImgH * captureAspect;
      }
      
      const xPos = (pageWidth - finalImgW) / 2;
      const yPos = titleSpace + (pdfAvailableH - finalImgH) / 2 + imgMargin;

      // Inserir imagem capturada
      pdf.addImage(dataUrl, 'JPEG', xPos, yPos, finalImgW, finalImgH);

      // 6. SALVAR ARQUIVO
      const timestamp = gerarTimestamp();
      pdf.save(`fluxograma-${projetoAtivo?.nome || 'projeto'}-${timestamp}.pdf`);
      
      addDebugLog('PDF v2.0.5 gerado com sucesso (Padronizado)!');
      setSyncStatus('synced');
    } catch (error) {
      console.error('Erro crítico na exportação PDF:', error);
      addDebugLog('Erro na exportação: ' + (error instanceof Error ? error.message : 'Erro desconhecido'));
      setSyncStatus('error');
      alert('Não foi possível gerar o PDF padronizado. Tente novamente em alguns instantes.');
    }
