...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF (v2.0.6 - Solução Anti-Quadrado Preto)...');
    setSyncStatus('syncing');
    ...
    try {
      ...
      // 2. CONFIGURAÇÃO DO "ESTÚDIO DE CAPTURA" (v2.0.6)
      const captureWidth = 2000;
      const captureHeight = 1125; // 16:9
      
      // Criamos um container que está no DOM e "visível" para o browser (opacidade 0.01)
      const vContainer = document.createElement('div');
      vContainer.id = 'pdf-capture-studio-v206';
      ...
      if (viewport) {
        ...
        const finalScale = Math.min(scaleW, scaleH, 1.2);
      ...
      // 5. CAPTURA USANDO CANVAS
      const canvas = await toCanvas(vContainer, {
        width: captureWidth,
      ...
      if (dataUrl.length < 20000) {
        throw new Error('Falha na captura da imagem (vazia).');
      }
      ...
      const availableH = pageHeight - (margin * 2) - titleSpace;
      ...
      const y = titleSpace + (availableH - imgH) / 2 + margin;
      ...
      addDebugLog('PDF v2.0.6 gerado com sucesso!');
      setSyncStatus('synced');
    } catch (error) {
      console.error('Erro na exportação PDF:', error);
      addDebugLog('Erro: ' + (error instanceof Error ? error.message : 'Erro desconhecido'));
      setSyncStatus('error');
