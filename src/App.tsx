...
const FlowContent = () => {
  ...
  const exportarPDF = useCallback(async () => {
    ...
    addDebugLog('Iniciando exportação PDF Profissional (v1.9.2)...');
    setSyncStatus('syncing');
    ...
    try {
      // 1. Calcular os limites reais de todos os nós
      const nodesBounds = getRectOfNodes(nodes);
      
      // 2. Definir dimensões para um "Canvas Virtual" de alta qualidade (Paisagem)
      // Garantimos um espaço mínimo de 1200px de largura para manter o aspecto desktop
      const padding = 80;
      const titleSpace = 100;
      const virtualWidth = Math.max(1200, nodesBounds.width + (padding * 2));
      const virtualHeight = nodesBounds.height + (padding * 2) + titleSpace;

      // 3. Preparar a UI (ocultar elementos que não devem sair no PDF)
      ...
      // 4. Forçar estilos de segurança nos labels das arestas
      ...
      // 5. Captura Inteligente: Forçamos o React Flow a se renderizar em um tamanho fixo
      // Isso ignora se o usuário está no celular ou desktop
      const dataUrl = await toPng(flowElement, {
        ...
        style: {
          ...
          // Centralizamos o conteúdo no canvas virtual
          transform: `translate(${(-nodesBounds.x + padding)}px, ${(-nodesBounds.y + padding + titleSpace)}px) scale(1)`,
        },
      });

      // 6. Restaurar a interface original
      ...
      // Adicionar Título diretamente no PDF (evita sobreposição e garante nitidez)
      ...
      // Restaurar a visão original do usuário
