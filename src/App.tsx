  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', overflow: 'hidden', flexDirection: isMobileView ? 'column' : 'row' }}>
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', position: 'relative' }}>
        <div style={{ 
          ...headerStyle, 
          height: isMobileView ? (isLandscape ? '48px' : 'auto') : '80px', 
          padding: isMobileView ? (isLandscape ? '0 12px' : '12px 16px') : '0 30px', 
          flexDirection: isMobileView ? (isLandscape ? 'row' : 'column') : 'row', 
          gap: isMobileView ? (isLandscape ? '8px' : '12px') : '0',
          minHeight: isMobileView && !isLandscape ? '100px' : 'auto'
        }}>
          ...
          <div style={{ 
            ...
            width: isMobileView && !isLandscape ? '100%' : 'auto' 
          }}>
            {isMobileView ? (
              <button onClick={() => setShowMobileMenu(true)} style={{ ...btnSecundario, padding: '6px' }}>
                <Layers size={18} />
              </button>
            ) : (
              <button onClick={() => { 
          ...
          <div style={{ 
            ...
            {modoEdicao ? (
              ...
              <div style={{ display: 'flex', flexDirection: isLandscape ? 'row' : 'column', alignItems: 'center', gap: isLandscape ? '8px' : '0', overflow: 'hidden', width: '100%' }}>
                <h1 style={{ 
                  fontSize: isMobileView ? (isLandscape ? '12px' : '14px') : '18px', 
                  fontWeight: 800, 
                  color: '#1e293b', 
                ...
                {isMobileView && (
                  <div style={{ 
                    ...
                    {isLandscape ? 'CONSULTA' : 'Modo Consulta Ativo'}
                  </div>
                )}
              </div>
            )}
          ...
          <div style={{ 
            ...
            alignItems: 'center', 
            flex: isLandscape ? '0 0 auto' : 1, 
            justifyContent: isMobileView && !isLandscape ? 'center' : 'flex-end', 
            width: isMobileView && !isLandscape ? '100%' : 'auto' 
          }}>
            <button onClick={() => fitView({ duration: 800, padding: 0.4 })} style={{ ...btnSecundario, padding: isMobileView ? '6px 10px' : '10px 20px', fontSize: isMobileView ? '10px' : '13px' }}>Visão Geral</button>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              {syncStatus === 'syncing' && <RefreshCw size={12} className="animate-spin" color="#3b82f6" />}
              {syncStatus === 'synced' && <Cloud size={12} color="#10b981" />}
              {syncStatus === 'error' && <CloudOff size={12} color="#ef4444" />}
              <span style={{ fontSize: '9px', fontWeight: 700, color: syncStatus === 'error' ? '#ef4444' : '#64748b' }}>
      ...
      {selecionado && (
        <div style={{
          ...sidePanel,
          width: isMobileView ? (isLandscape ? '260px' : '100%') : '300px',
          height: isMobileView && !isLandscape ? '50vh' : 'auto',
          top: isMobileView ? (isLandscape ? '58px' : 'auto') : '96px',
          ...
          borderRadius: isMobileView && !isLandscape ? '20px 20px 0 0' : '18px',
          padding: isMobileView ? '20px' : '28px',
          boxShadow: isMobileView ? '0 -10px 25px rgba(0,0,0,0.1)' : '0 0 30px rgba(0,0,0,0.08)',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isMobileView ? '15px' : '25px' }}>
            <h3 style={{ fontSize: isMobileView ? '12px' : '14px', fontWeight: 700 }}>
