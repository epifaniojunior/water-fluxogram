      ...
      <div style={{ padding: '12px', background: 'white', textAlign: 'center', borderTop: '1px solid #f1f5f9', minHeight: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
        ...
        {data.tipo === 'Tratamento' && (data.concessionaria || data.uc || data.medidor) && (
          <>
            ...
            <div style={{ fontSize: '10px', color: '#334155', textAlign: 'left', width: '100%', display: 'flex', flexDirection: 'column', gap: '2px', textTransform: 'uppercase' }}>
              {data.concessionaria && <div><span style={{fontWeight: 800}}>Css.</span> {data.concessionaria}</div>}
              {data.uc && <div><span style={{fontWeight: 800}}>UC</span> {data.uc}</div>}
              {data.medidor && <div><span style={{fontWeight: 800}}>Med.</span> {data.medidor}</div>}
            </div>
          </>
        )}
      </div>
